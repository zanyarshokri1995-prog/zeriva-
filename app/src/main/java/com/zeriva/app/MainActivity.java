rivate final int GREEN = Color.rgb(12, 45, 31);
...
private void showHome() {
    ...
    main.addView(new ZerivaLogoView(this),
...
private void addCustomer() {
    ...
    button(l, "ذخیره مشتری", v -> {
        ...
        x.put("name", n);
        x.put("phone", p);
        db.getWritableDatabase().insert("customers", null, x);
...
private void customerList() {
    ...
    while (c.moveToNext()) {
        ...
        l.addView(card(
                ...
                "\nنام: " + name +
                "\nتلفن: " +
                (phone == null || phone.isEmpty() ? "ثبت نشده" : phone)
        ));
    }
    c.close();
...
private void customerByCity() {
    ...
    Cursor c = db.getReadableDatabase().rawQuery(
            "SELECT o.city, o.customer_no, o.full_name, o.phone " +
            "FROM orders o " +
            "INNER JOIN (" +
            " SELECT customer_no, MAX(id) AS max_id " +
            " FROM orders GROUP BY customer_no" +
            ") latest ON o.customer_no=latest.customer_no AND o.id=latest.max_id " +
            "ORDER BY COALESCE(NULLIF(o.city,''),'بدون شهر'), o.full_name",
            null);
    ...
    while (c.moveToNext()) {
        ...
        String cityName = (city == null || city.trim().isEmpty()) ? "بدون شهر" : city;

        if (lastCity == null || !lastCity.equals(cityName)) {
...
private void dailyPrice() {
    ...
    l.addView(card("قیمت فعلی هر کیلو:\n" + money(current) + " تومان"));
    ...
    button(l, "ثبت قیمت روز", v -> {
        ...
        d.delete("settings", "key=?", new String[]{"daily_price"});
...
private double getDailyPrice() {
    Cursor c = db.getReadableDatabase().rawQuery(
            "SELECT value FROM settings WHERE key='daily_price' LIMIT 1",
            null);
...
private void updateUndeliveredOrdersPrice(double price) {
    SQLiteDatabase d = db.getWritableDatabase();
    ContentValues x = new ContentValues();
    x.put("daily_price", price);
    x.put("total_amount", 0);
    x.put("balance", 0);

    // ابتدا مبلغ‌ها را با SQL محاسبه می‌کنیم تا وزن و بیعانه هر سفارش حفظ شود.
    d.execSQL(
            "UPDATE orders SET " +
            "daily_price=?, " +
            "total_amount=kg*?, " +
...
private void addOrder() {
    ...
    orderPriceView = card(
            "قیمت روز هر کیلو: " + money(getDailyPrice()) + " تومان");
    l.addView(orderPriceView);

    button(l, "🔄 بروزرسانی قیمت روز", v -> refreshOrderPrice());
    ...
    button(l, "ثبت سفارش", v -> {
        ...
        double weight = number(kg.getText().toString());
        double dep = number(deposit.getText().toString());
        ...
        int customerNo = createCustomerIfNeeded(n, p);
        ...
        db.getWritableDatabase().insert("orders", null, x);
...
private void refreshOrderPrice() {
    if (orderPriceView == null) return;

    double currentPrice = getDailyPrice();
    orderPriceView.setText(
            "قیمت روز هر کیلو: " + money(currentPrice) + " تومان");
    toast("قیمت روز: " + money(currentPrice) + " تومان");
}

private int createCustomerIfNeeded(String name, String phone) {
    SQLiteDatabase d = db.getWritableDatabase();
    ...
    x.put("name", name);
    x.put("phone", phone);
    long id = d.insert("customers", null, x);
    return (int) id;
}
...
private void orderList() {
    ...
    Cursor c = db.getReadableDatabase().rawQuery(
            "SELECT id,customer_no,full_name,phone,province,city,address," +
            "grape_type,kg,daily_price,total_amount,deposit,balance," +
            "description,delivered FROM orders ORDER BY id DESC",
            null);
    ...
    while (c.moveToNext()) {
        final int orderId = c.getInt(0);
        int customerNo = c.getInt(1);
        String name = c.getString(2);
        String phone = c.getString(3);
        String province = c.getString(4);
        String city = c.getString(5);
        String address = c.getString(6);
        String grape = c.getString(7);
        double kg = c.getDouble(8);
        double price = c.getDouble(9);
        double total = c.getDouble(10);
        double deposit = c.getDouble(11);
        double balance = c.getDouble(12);
        String description = c.getString(13);
        boolean delivered = c.getInt(14) == 1;
        ...
        l.addView(card(
                ...
                "\nنوع انگور: " + grape +
                "\nوزن: " + money(kg) + " کیلو" +
                "\nقیمت ثبت‌شده: " + money(price) + " تومان" +
                "\nمبلغ کل: " + money(total) + " تومان" +
                "\nبیعانه: " + money(deposit) + " تومان" +
                "\nمانده: " + money(balance) + " تومان" +
                "\nوضعیت: " + (delivered ? "تحویل شده" : "در انتظار تحویل") +
                "\nتوضیحات: " +
                (description == null || description.isEmpty() ? "-" : description)
        ));

        if (!delivered) {
            button(l, "✅ تحویل شد - سفارش " + customerNo,
...
private void markDelivered(int orderId) {
    ...
    db.getWritableDatabase().update(
            "orders", x, "id=?", new String[]{String.valueOf(orderId)});

    toast("سفارش تحویل‌شده ثبت شد");
    orderList();
}
...
private void addTransaction() {
    ...
    button(l, "ذخیره معامله", v -> {
        String customerNo = customer.getText().toString().trim();
        String desc = description.getText().toString().trim();
        double dValue = number(debt.getText().toString());
        double pValue = number(payment.getText().toString());
        ...
        db.getWritableDatabase().insert("transactions", null, x);
...
private void transactionList() {
    ...
    Cursor c = db.getReadableDatabase().rawQuery(
            ...
            "FROM transactions ORDER BY id DESC", null);
...
private void purchase() {
    ...
    button(l, "ذخیره خرید", v -> {
        ...
        db.getWritableDatabase().insert("purchases", null, x);
        toast("خرید ذخیره شد");
        showHome();
    });
...
private void sale() {
    ...
    button(l, "ذخیره فروش", v -> {
        ...
        db.getWritableDatabase().insert("sales", null, x);
        toast("فروش ذخیره شد");
        showHome();
    });
...
private void reports() {
    ...
    double deposits = sum(d, "orders", "deposit");
    double orderBalance = sum(d, "orders", "balance");

    l.addView(card(
            "📦 خرید\nمقدار: " + money(purchasedKg) +
            " کیلو\nمبلغ: " + money(purchases) + " تومان"));

    l.addView(card(
            "🚚 فروش\nمقدار: " + money(soldKg) +
            " کیلو\nمبلغ: " + money(sales) + " تومان"));

    l.addView(card(
            "🛒 سفارش‌ها\nبیعانه دریافت‌شده: " +
            money(deposits) + " تومان\nمانده سفارش‌ها: " +
            money(orderBalance) + " تومان"));

    l.addView(card(
            "💰 فروش منهای خرید\n" +
            money(sales - purchases) + " تومان"));

    button(l, "📍 میزان سفارش هر شهر", v -> cityOrderReport());
    button(l, "بازگشت", v -> showHome());
...
private void cityOrderReport() {
    LinearLayout l = layout();
    l.addView(title("میزان سفارش بر اساس شهر", 30));

    Cursor c = db.getReadableDatabase().rawQuery(
            "SELECT COALESCE(NULLIF(city,''),'بدون شهر') AS city_name, " +
            "COALESCE(SUM(kg),0) AS total_kg, " +
            "COUNT(*) AS order_count " +
            "FROM orders GROUP BY city_name ORDER BY total_kg DESC",
            null);
    ...
    while (c.moveToNext()) {
        ...
        int orders = c.getInt(2);
        allKg += kg;
        rows++;

        l.addView(card(
                "🏙️ " + city +
                "\nمجموع سفارش: " + money(kg) + " کیلو" +
                "\nتعداد سفارش: " + orders
        ));
    }
    ...
    if (rows == 0) {
        ...
        l.addView(card("📊 مجموع کل سفارش‌ها:\n" +
                money(allKg) + " کیلو"));
    }

    button(l, "بازگشت", v -> reports());
    setPage(l);
}
...
private LinearLayout layout() {
    ...
    l.setPadding(dp(25), dp(30), dp(25), dp(25));
...
private TextView text(String s) {
    ...
    t.setTextSize(19);
    t.setGravity(Gravity.CENTER);
    t.setPadding(dp(10), dp(25), dp(10), dp(25));
    return t;
}
...
private TextView card(String s) {
    ...
    t.setPadding(dp(15), dp(15), dp(15), dp(15));
...
private EditText input(String hint) {
    ...
    e.setPadding(dp(15), dp(8), dp(15), dp(8));
...
private void button(LinearLayout parent, String text, View.OnClickListener click) {
...
private double sum(SQLiteDatabase d, String table, String column) {
    Cursor c = d.rawQuery(
            "SELECT COALESCE(SUM(" + column + "),0) FROM " + table, null);
    double result = 0;
    if (c.moveToFirst()) result = c.getDouble(0);
    c.close();
    return result;
...
// لوگوی وکتوری ZERIVA؛ بدون نیاز به فایل عکس جداگانه
private class ZerivaLogoView extends View {
    private final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);

    ZerivaLogoView(android.content.Context context) {
        super(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setColor(GOLD);
        canvas.drawCircle(cx, cy, dp(68), paint);

        paint.setStyle(Paint.Style.FILL);

        // ساقه
        paint.setColor(GOLD);
        canvas.drawRect(cx - dp(3), cy - dp(55),
                cx + dp(3), cy - dp(25), paint);

        // برگ
        canvas.save();
        canvas.rotate(-28, cx + dp(12), cy - dp(45));
        canvas.drawOval(cx + dp(3), cy - dp(57),
                cx + dp(38), cy - dp(36), paint);
        canvas.restore();
        ...
        for (int row = 0; row < 5; row++) {
            int count = row + 1;
            float startX = cx - ((count - 1) * gap) / 2f;
            for (int col = 0; col < count; col++) {
                float x = startX + col * gap;
                float y = cy - dp(18) + row * dp(12);
        ...
        canvas.drawText("MARIWAN • ZARIVAR", cx, cy + dp(56), paint);
    }
}

private class DB extends SQLiteOpenHelper {
    DB() {
        super(MainActivity.this, "ZERIVA.db", null, 3);
    }
    ...
    private void createNewTables(SQLiteDatabase d) {
        d.execSQL(
    ...
    public void onUpgrade(SQLiteDatabase d, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 3) {
            try {
                ...
                // اگر ستون قبلاً وجود داشته باشد، اطلاعات حفظ می‌شود.
            }
        }
    }
}
} ''' path = "/mnt/data/MainActivity_ZERIVA_Complete.java
